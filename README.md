...
- **Cached.** Refreshes the repo list once an hour by default (`GHFZF_TTL`
  to override, `--refresh` or `Ctrl-R` in the picker to force).
...
ghj --refresh        # same, but refresh the repo cache first
```
...
ghfzf --refresh         # force-refresh the repo cache
...
| `GHFZF_TTL`   | `3600`             | Cache freshness, in seconds                               |
...
The repo list is cached for an hour. Press `Ctrl-R` in the picker or run
`ghj --refresh` / `ghfzf --refresh`.
